...
do
	ssh -f ${USER_TRAFFIC_GENERATOR}@${TRAFFIC_GENERATOR_IP} "/${OUTPUT_DIRECTORY}/${WORKER_GENERATOR_FOLDER}/${WORKER_FOLDER}${i}/${WORKER_BIN_NAME} -t -i ${SERVER_IP} -p ${SERVER_PORT} -d ${DELAY_MESSAGES}"
done
